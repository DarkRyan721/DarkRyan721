...
fs.mkdirSync('assets/rows-b', { recursive: true });

const manifest = JSON.parse(fs.readFileSync('assets/photos/manifest.json', 'utf8'));
const counts = { row1: {}, row2: {} };

for (const t of ['light', 'dark']) {
  const skills = inner(`assets/skills-v4-${t}.svg`);
  const profile = inner(`assets/profile-v4-${t}.svg`);
  ...
  manifest.wide[t].forEach((photo, i) => {
    ...
    fs.writeFileSync(`assets/rows-b/row1-${t}-${i}.svg`, svg);
  });
  ...
  manifest.tall[t].forEach((photo, i) => {
    ...
    fs.writeFileSync(`assets/rows-b/row2-${t}-${i}.svg`, svg);
  });
  counts.row2[t] = manifest.tall[t].length;
}

fs.writeFileSync('assets/rows-b/manifest.json', JSON.stringify(counts, null, 2));
